...
namespace geom {
    ...
    namespace ch {
        tuple<vector<point>, vector<point>> graham(const vector<point> &_P);
        int quick(vector<point> &P);
    }
    ...
    tuple<vector<point>, vector<point>> ch::graham(const vector<point> &_P) {
        auto P = _P;
        auto n = (int) P.size();

        if (n <= 1) {
            return make_tuple(P, vector<point>());
        }
...
void solve() {
    ...
    using geom::op::angle;
    auto m = quick(P);
    double a = (m >= 3 ? min(angle(P[m - 1], P[0], P[m - 2]),
                             angle(P[0], P[1], P[m - 1])) : 0.);
    forn(i, m - 2) {
        a = min(a, angle(P[i + 1], P[i + 2], P[i]));
    }
    cout << setprecision(10) << ((long double) a) * 180 / M_PI;
    // io::writeDouble(a * 180 / M_PI, 9);
}